...
そしてこの提案手法を実装し,テストベッドおよびパブリックネットワーク上で評価した.
...
In this way, by utilizing the fact that the same contents are arranged in various places, simultaneous concurrent multiplexing communication with a plurality of web servers is carried out, so that various technologies to realize higher speed and comfortable communication vary Have been developed and proposed in places.
In this research, in order to deal with the known problem when utilizing the same file while acquiring the same file by using plural TCP connections with different performances, while comparing the performance between each TCP connection, occurrence of inversion order inversion We propose an active HTTP progressive download order control method at request transmission aimed at suppressing download progress.
We implemented this proposed method and evaluated it on test bed and public network.
...
In addition, in the evaluation on the public network, the delay prediction method using the difference measurement is combined with the initial delay prediction method to reduce the initial buffering time by 50\%, the ineffectiveness of 30\% We were able to confirm the reduction of the number of blocks and the reduction of the average delay time by 50\%.
}
\begin{document} 
\maketitle %とびらの出力
