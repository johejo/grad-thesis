	...
	\begin{center}
		\begin{tabular}{cc}
			\begin{minipage}[t]{0.9\hsize}
				\includegraphics[width=13.5cm]{figure/NumberofBlocksStayinginBuffer.pdf}
				\caption{平均非有効ブロック数}
				\label{nsb}
			\end{minipage}\\ \\
			\begin{minipage}[t]{0.9\hsize}
				\includegraphics[width=13.5cm]{figure/AverageDelayTime.pdf}
				\caption{平均遅延時間}
				\label{adt}
			\end{minipage}\\
		\end{tabular}
...
固定遅延要求方式における平均非有効ブロック数が0でなく0.5に近い値である理由として,
標準偏差が非常に小さいことから,
ブロック到着時刻の微妙なずれからある時刻に置いて非有効ブロック数が1となる場合と,
到着順序逆転が発生せず非有効ブロック数が0である場合がほぼ同等に発生している可能性が挙げられる.
初期遅延予測を行わない場合は,初期リクエスト時において非有効ブロックが生じていることも確認できる.

図\ref{adt}より,平均遅延時間にについても,固定遅延要求方式とほぼ同等の値が得られている.

また,本テストベッド環境のようなネットワーク環境の変化が生じにくい環境では,
遅延要求を行うことで非有効ブロックの発生が抑えられるので,
結果として重複再要求を行うことが殆どないと言える.
よって遅延要求を行うことで冗長な要求によるネットワーク帯域の使用も減らすことができ,
...
また,10回のすべての試行での各ブロック番号における非有効ブロック数と遅延時間をプロットしたものを図\ref{allnsb},図\ref{alladt}に示す.
\begin{figure}[h]
    \begin{tabular}{cc}
       	\begin{minipage}[t]{0.9\hsize}
       		\centering
...
\begin{figure}[ht]
	\begin{tabular}{cc}
		\begin{minipage}[t]{0.9\hsize}
			\centering
			\includegraphics[width=12cm]{figure/TypicalPlotDelay=NORMALInit=FalseDup=IBRC.pdf}
			\caption{遅延要求なしの場合の受信の様子}
			\label{tpdnpub}
		\end{minipage}\\ \\
		\begin{minipage}[t]{0.9\hsize}
			\centering
			\includegraphics[width=12cm]{figure/TypicalPlotDelay=DIFFInit=TrueDup=IBRC.pdf}
