...
実装はPythonで行った.
まず,入力として複数のホストのURL,分割サイズ等を入力する.
プログラムはURLが有効であるかをHTTP HEADリクエストを用いて確認する.
入力された分割サイズを元にブロック1個あたりのバイト数を決定し,
HTTP-Range リクエストに必要なヘッダーを生成してキューに保管する.
便宜上キューと呼称しているが、このキューは任意の位置のデータを取り出すことのできるように改良してある
次にホストとの接続を確立する.
ここでは既存のHTTPライブラリをセッション層に利用している.
次にホスト数の子スレッドを起動し,ファイル全体が揃うまで各スレッドがキューからヘッダーをとりだして各ホストに要求を送信し,ブロックの受信が終了したら親スレッドにブロック受信完了を通知し,スレッド共有のメモリ上に保管するという作業を繰り返す.
親スレッドはファイル全体の返却が終了するまで,子スレッドからの通知を受け取ると共有メモリを確認して順序が揃った部分を連結してユーザーに返却する,
という作業を繰り返す.
この返却についてはジェネレータオブジェクトを利用することで,
ユーザーは任意のタイミングでデータを読み出すことができる.
